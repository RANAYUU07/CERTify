...
export default function App() {
  ...
  const renderActiveTab = () => {
    ...
    if (activeTab === "confirm")
      return <HumanConfirmation key="confirmation" />;
              ...
              <div>
                ...
                <div className="relative flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-background border-b-2 border-border focus:border-accent font-mono text-sm py-2 px-0 outline-none"
                  />
                  {apiKey && (
                    <button
                      onClick={() => setApiKey("")}
                      className="text-muted-foreground hover:text-severity-critical text-xs font-bold uppercase tracking-widest flex-shrink-0"
                ...
                <p className="text-[0.6rem] text-muted-foreground mt-2 uppercase tracking-wider">
                  Stored in-memory for current session only. Key is hidden and
                  cannot be copied — clear it to enter a new one.
                </p>
              </div>
